...
make_rand_plot <- function(split_subgroups = TRUE, return_what) {
  set.seed(42)
  grp1 <- noisy_line(input$mlow, input$blow, input$nlow, '1 (Low risk)')
  grp2 <- noisy_line(input$mmed, input$bmed, input$nmed, '2 (Medium risk)')
  grp3 <- noisy_line(input$mhi, input$bhi, input$nhi, '3 (High risk)')
